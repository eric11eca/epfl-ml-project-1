...
We tested the models on 5-fold cross validation for finding the best hyperparameters (`cv_params=gamma,lambda_`), which returns k-final weights, k-best weights, and logs includes training loss, validation loss, train accuracy, validation accuracy, validation f1 score, validation precision, and validation recall.
